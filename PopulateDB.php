...
class PopulateDB {
    ...
    public function __construct(
        ...
        private string $dbname = "scandiweb-test"
    ) {
    ...
    private function createTables(){
        ...
        $prices_table = "CREATE TABLE IF NOT EXISTS Product_Prices (
            ID INT AUTO_INCREMENT PRIMARY KEY,
            Product_ID VARCHAR(255),
            Currency_Label VARCHAR(5),
            Currency_Symbol VARCHAR(3),
            Currency__Typename VARCHAR(10),
            Amount DECIMAL(10,2),
            __typename VARCHAR(25),
            FOREIGN KEY (Product_ID) REFERENCES Products(ID)
        )";

        $tables = [$orders_table, $categories_table, $products_table, $gallery_table, $attributes_table, $items_table, $prices_table];
    ...
    private function populateDB(){
        ...
        foreach($data['data']['products'] as $product){
            ...
            $gallery_query = $this->conn->prepare(
                "INSERT INTO Products_gallery(Product_ID, URL) VALUES (?,?)"
            );
            ...
            foreach($product["attributes"] as $attribute){
                ...
                $i_item_id = $this->conn->insert_id;

                $item_query = $this->conn->prepare(
                    "INSERT INTO Attribute_Items (ID, Attribute_ID, Display_Value, Item_Value, __typename) VALUES (?,?,?,?,?)"
                );

                $item_query->bind_param("sisss", $item_id, $i_item_id, $display_value, $item_value, $item__typename);
            ...
            $price_query = $this->conn->prepare(
                "INSERT INTO Product_Prices (Product_ID, Currency_Label, Currency_Symbol, Currency__Typename, Amount, __typename) VALUES (?,?,?,?,?,?)"
            );
            $price_query->bind_param("ssssds", $product_id, $currency_label, $currency_symbol, $currency__typename, $amount, $price__typename);
